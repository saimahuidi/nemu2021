...
static def_DHelper(J) {
  sword_t simm = (s->isa.instr.j.simm20 << 20) | (s->isa.instr.j.imm19_12 << 12) | (s->isa.instr.j.imm10_1 << 1) | (s->isa.instr.j.imm11 << 11);
  decode_op_i(s, id_src1, simm, true);
  decode_op_r(s, id_dest, s->isa.instr.u.rd, true);
}

static def_DHelper(B) {
  sword_t simm = (s->isa.instr.b.simm12 << 12) | (s->isa.instr.b.imm11 << 11) | (s->isa.instr.b.imm4_1 << 1) | (s->isa.instr.b.imm10_5 << 5);
  decode_op_i(s, id_src2, simm, true);
  decode_op_r(s, id_dest, s->isa.instr.b.rs1, false);
  decode_op_r(s, id_src1, s->isa.instr.b.rs2, false);
}
...
def_THelper(main) {
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I     , 0010011);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 01100 11", R     , 0110011);
